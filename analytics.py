...
def show_analytics(conn, texts):
    ...
    if complaints_data:
        ...
        with row1_col2:
            fig_pri = px.pie(df, names="priority", title="Complaints by Priority", color_discrete_sequence=px.colors.sequential.Blues)
        ...
        with row2_col1:
            fig_status = px.pie(df, names="status", title="Complaints by Status", color_discrete_sequence=px.colors.sequential.Greens)
        ...
        with row2_col2:
            ...
            fig_time.update_traces(line_shape="linear")  # تم تصحيح مكان line_shape
